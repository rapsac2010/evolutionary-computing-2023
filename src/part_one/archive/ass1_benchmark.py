...
def mu_comma_lambda_selection(population, offspring, fitness=None):
...
def mu_plus_lambda_selection(population, offspring, fitness):
    """Performs mu + lambda selection by selecting the best individuals from
    both parents and offspring"""
    fitness_new = evaluate(offspring)
    fitness = np.hstack([fitness, fitness_new])
...
def blx_alpha_crossover(population, fitness, alpha=0.5, offspring_multiplier=2):
    ...
    while count < offspring_shape[0]:
        # Parent selection
        parents_idx = select_parents_tournament(
            population, fitness, 2, K=population.shape[0] / 4
        )
        parents = population[parents_idx, :]
        ...
        for j in range(parents.shape[1]):
            ...
            offspring[count, j] = gamma * np.max(parents[:, j]) + (1 - gamma) * np.min(
                parents[:, j]
            )
...
def simple_arithmetic_crossover(population, fitness, alpha=0.5, offspring_multiplier=2):
    ...
    while count < offspring_shape[0]:
        # Parent selection
        parents_idx = select_parents_tournament(
            population, fitness, 2, K=population.shape[0] / 2
        )
        parents = population[parents_idx, :]
...
def reset_coefficients_mutation(population, p=0.1):
    ...
    reset_array = initialize_population(
        POP_SIZE=population.shape[0], N_VARS=population.shape[1]
    )
...
def total_reset_mutation(population, p=0.1):
    ...
    mutated = (
        np.matmul(population.T, np.diag(mask)).T
        + np.matmul(reset_array.T, np.diag(~mask)).T
    )
    return mutated
...
def combine_operators(
    ...
    for i, (mutation, crossover) in enumerate(
        itertools.product(MUTATION_OPERATORS, CROSSOVER_OPERATORS)
    ):
        ...
        mutated = mutation(
            crossover(temp_pop, temp_fit, offspring_multiplier=offspring_multiplier)
        )
...
def main_game_loop(env, n_generations=50, initial_pop=None):
    ...
    if not initial_pop:
        ...
        pop = initialize_population(N_VARS=N_VARS, POP_SIZE=40)
    else:
        pop = initial_pop
    ...
    for i in range(n_generations):
        ...
        pop, fitness = mu_comma_lambda_selection(pop, offspring, fitness)
