...
def initialize_sigmas(LOWER=0.1, UPPER=2, POP_SIZE=100):
    """Randomly initialize an array of sigma values"""
    sigmas = np.random.uniform(LOWER, UPPER, (POP_SIZE, 1))
    return sigmas
...
def mu_comma_lambda_selection(
    env, population, sigmas, offspring, sigmas_offspring, fitness, keep_best=True
):
    ...
    if keep_best:
        ...
        sigmas_offspring = np.vstack(
            [sigmas[best_idx, :], sigmas_offspring[selected, :][:-1]]
        )
        ...
        sigmas_offspring = sigmas_offspring[selected, :]

    return offspring, sigmas_offspring, fitness_offspring
...
def whole_arithmetic_crossover(
    population, sigmas, fitness, alpha=0.5, offspring_multiplier=2, k=4
):
    ...
    sigmas_offspring = np.zeros(shape=(offspring_shape[0], 1))
    ...
    while count < offspring_shape[0]:
        ...
        sigmas_parents = sigmas[parents_idx, :]
        ...
        for _ in range(offspring.shape[0]):
            ...
            sigmas_offspring[count, :] = sigmas_parents[0, :]
            count += 1
...
# MUTATION OPERATORS
#########################
def additive_gaussian_mutation(
    population, sigmas, p=0.2, per_individual=False, learning_rate=0.1
):
    """Add random noise to the weights of the offspring,
    an individual has probability = p of being mutated
    """
    sigma_matrix = np.hstack([sigmas] * population.shape[1])
    noise = np.random.normal(0, scale=sigma_matrix, size=population.shape)
    ...
    sigmas = sigmas * np.exp(
        np.random.normal(0, scale=learning_rate, size=sigmas.shape)
    )
    return mutated, sigmas
...
def combine_operators(
    population, sigmas, fitness, mutation_operator, crossover_operator
):
    """Combines mutation and crossover to generate offspring"""
    population, sigmas = crossover_operator(population, sigmas, fitness)
    offspring, sigmas_offspring = mutation_operator(population, sigmas)
    return offspring, sigmas_offspring
...
def main_game_loop(
    ...
    for i in range(N_GENERATIONS + 1):
        ...
        offspring, sigmas_offspring = combine_operators(
            population=pop,
            sigmas=sigmas,
            fitness=fitness,
        ...
        pop, sigmas, fitness = mu_comma_lambda_selection(
            env, pop, sigmas, offspring, sigmas_offspring, fitness, keep_best=KEEP_BEST
        )
...
if __name__ == "__main__":
    ...
    experiment_dir = Path("ass1_dynamic_experiments")
    experiment_dir.mkdir(exist_ok=True)
    experiment_name = f"dynamic_enemy{enemies[0]}_{date_str()}"
